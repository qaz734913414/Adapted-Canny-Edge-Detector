...
/* ECPS 203 Assignment 1 solution*/
...
{  unsigned char tempimage[76800];
   unsigned char tempedge[76800];
   char *infilename = NULL;  /* Name of the input image */
   char *dirfilename = NULL; /* Name of the output gradient direction image */
   char outfilename[128];    /* Name of the output "edge" image */
   char composedfname[128];  /* Name of the output "direction" image */
   unsigned char *image = &tempimage[0];     /* The input image */
   unsigned char *edge = &tempedge[0];      /* The output edge image */
...
void canny(unsigned char *image, int rows, int cols, float sigma,
         float tlow, float thigh, unsigned char **edge, char *fname)
{
   unsigned char tempnms[76800];
   short int tempsmoothedim[76800];
   short int tempdeltax[76800];
   short int tempdeltay[76800];
   short int tempmagnitude[76800];
   FILE *fpdir=NULL;          /* File to write the gradient image to.     */
   unsigned char *nms = &tempnms[0];        /* Points that are local maximal magnitude. */
   short int *smoothedim = &tempsmoothedim[0],     /* The image after gaussian smoothing.      */
             *delta_x = &tempdeltax[0],        /* The first devivative image, x-direction. */
             *delta_y = &tempdeltay[0],        /* The first derivative image, y-direction. */
             *magnitude = &tempmagnitude[0];      /* The magnitude of the gadient image.      */
        ...
        short int **smoothedim)
{
   ...
   float *tempim,        /* Buffer for separable filter gaussian smoothing. */
         *kernel = &tempkernel[0],        /* A one dimensional gaussian kernel. */
	...
	float tlow, float thigh, unsigned char *edge)
{
   int r, c, pos, numedges, /*lowcount,*/ highcount, lowthreshold, highthreshold,
      /*i,*/ hist[32768]/*, rr, cc*/;
   short int maximum_mag = 0;/*, sumpix*/;
    ...
    int *cols)
{
   ...
   * Allocate memory to store the image then read the image from the file.
   ***************************************************************************/
   if(((*image_red) = (unsigned char *) ((*rows)*(*cols))) == NULL){
      fprintf(stderr, "Memory allocation failure in read_ppm_image().\n");
      if(fp != stdin) fclose(fp);
      return(0);
   }
   if(((*image_grn) = (unsigned char *) ((*rows)*(*cols))) == NULL){
      fprintf(stderr, "Memory allocation failure in read_ppm_image().\n");
      if(fp != stdin) fclose(fp);
      return(0);
   }
   if(((*image_blu) = (unsigned char *) ((*rows)*(*cols))) == NULL){
      fprintf(stderr, "Memory allocation failure in read_ppm_image().\n");
      if(fp != stdin) fclose(fp);
      return(0);
   }
