...
//Added for multi-threading in y
        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
   ...
   /****************************************************************************
   * Blur in the x - direction, with pthread multithreading. Edited by Jiang Wan
   ****************************************************************************/
...
//Added for multi-threading in y
  ...
  int j=0;
   ...
   for(j=0; j<N_T; j++)
   {
      args_y[j].rows = rows;
      args_y[j].cols = cols;
      args_y[j].row_s = row_per_t*j;
      args_y[j].row_e = row_per_t*(j+1);
      args_y[j].center = center;
      args_y[j].kernel = kernel;
      args_y[j].tempim = tempim;
      args_y[j].smoothedim = smoothedim;
      iret_y[j] = pthread_create(&thread_y[j], NULL, &blur_y, &args_y[j]);
    }

    for(j=0; j<N_T; j++)
    {
     pthread_join(thread_y[j], NULL);
    }
